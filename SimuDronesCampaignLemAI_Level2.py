...
# Simulation: Collective Robotics Defense with AI and EMP weapons.
...
def decide_emp_use(shooter: DroneUnit, state: DroneScenarioState) -> List[DroneUnit]:
    """Return list of targets affected by EMP if shooter decides to fire, else []."""
    if not CONFIG.get("ENABLE_EMP_WEAPONS", False):
        return []
    if not shooter.has_emp:
        return []
    ...
    close_threats = [
        e for e in in_range
        if shooter.distance_to(e) <= emp_range * 0.6
    ]

    should_fire = False
    if len(in_range) >= min_targets:
        should_fire = True
    elif shooter.hp < 40 and close_threats:
        should_fire = True
    ...
    while ds.turn_index < ds.max_turns:
        ...
        if CONFIG.get("ENABLE_EMP_WEAPONS", False):
            for shooter in ds.drones:
                ...
                targets_emp = decide_emp_use(shooter, ds)
                if not targets_emp:
                    continue
...
if __name__ == "__main__":
    ...
    CONFIG["ENABLE_EMP_WEAPONS"] = True  # ou False pour comparer
